...
$head=false;$mock=static function($pre,$args)use(&$head){$head=$args['method']==='HEAD';return ['headers'=>['location'=>'https://example.org/'],'body'=>'','response'=>['code'=>302,'message'=>'Found'],'cookies'=>[]];};add_filter('pre_http_request',$mock,10,2);$ref=new ReflectionClass(GTLM_Admin::class);$admin=$ref->newInstanceWithoutConstructor();gtlm_test_access($ref->getProperty('db'))->setValue($admin,$db);gtlm_test_access($ref->getProperty('settings'))->setValue($admin,$settings);$method=gtlm_test_access($ref->getMethod('run_diagnostics'));$method->invoke($admin);remove_filter('pre_http_request',$mock,10);retention_check($head,'Runtime diagnostics uses HEAD rather than adding a tracked GET');
