...
$event=$wpdb->get_row('SELECT * FROM '.GTLM_Analytics_DB::events_table().' ORDER BY id DESC LIMIT 1',ARRAY_A);verify($event['source']==='search.example.org'&&$event['country']==='IN'&&$event['device']==='mobile'&&$event['campaign']==='7','Event fields normalized',$event);verify(!str_contains(json_encode($event),'secret@')&&!str_contains(json_encode($event),'private/path')&&!str_contains(json_encode($event),'Mozilla'),'No raw referrer or user agent retained');
