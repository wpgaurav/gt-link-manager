...
$total=(int)$wpdb->get_var("SELECT SUM(clicks) FROM ".GTLM_Analytics_DB::hourly_table()." WHERE page_key='' AND dimension='total'");verify($total===2,'Aggregation matches independent event total',$total);GTLM_Analytics::process();verify((int)$wpdb->get_var("SELECT SUM(clicks) FROM ".GTLM_Analytics_DB::hourly_table()." WHERE page_key='' AND dimension='total'")===$total,'Repeated maintenance does not double count');
$groups=$wpdb->get_results('SELECT dimension,SUM(clicks) clicks FROM '.GTLM_Analytics_DB::hourly_table()." WHERE page_key='' GROUP BY dimension",ARRAY_A);verify(count(array_filter($groups,fn($g)=>(int)$g['clicks']!==$total))===0,'All breakdowns reconcile with total',$groups);
...
$hosts=(int)$wpdb->get_var("SELECT COUNT(DISTINCT value) FROM ".GTLM_Analytics_DB::hourly_table()." WHERE page_key='' AND dimension='source' AND value<>'_other' AND value<>''");verify($hosts===100,'Source cardinality cap persists across batches',$hosts);
...
$before=(int)$wpdb->get_var("SELECT SUM(clicks) FROM ".GTLM_Analytics_DB::hourly_table()." WHERE page_key='' AND dimension='total'");$adb->append($event);GTLM_Analytics::process();$late->commit();$late->close();GTLM_Analytics::process();verify((int)$wpdb->get_var("SELECT SUM(clicks) FROM ".GTLM_Analytics_DB::hourly_table()." WHERE page_key='' AND dimension='total'")===$before+2,'Late lower ID is processed without loss');
