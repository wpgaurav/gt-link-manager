...
class GTLM_REST_API {
	...
	public function delete_link( WP_REST_Request $request ) {
		...
		if ( $id <= 0 ) {
			return new WP_Error( 'gtlm_invalid_id', __( 'Invalid link ID.', 'gt-link-manager' ), array( 'status' => 400 ) );
		}

		if ( $force ) {
			$ok = $this->db->delete_link( $id );
			if ( ! $ok ) {
				return new WP_Error( 'gtlm_delete_failed', __( 'Could not delete link.', 'gt-link-manager' ), array( 'status' => 500 ) );
			}
		...
		if ( ! $ok ) {
			return new WP_Error( 'gtlm_trash_failed', __( 'Could not trash link.', 'gt-link-manager' ), array( 'status' => 500 ) );
		}
	...
	public function restore_link( WP_REST_Request $request ) {
		...
		if ( ! $ok ) {
			return new WP_Error( 'gtlm_restore_failed', __( 'Could not restore link.', 'gt-link-manager' ), array( 'status' => 500 ) );
		}
	...
	public function toggle_link_active( WP_REST_Request $request ) {
		...
		if ( ! $ok ) {
			return new WP_Error( 'gtlm_toggle_failed', __( 'Could not update link status.', 'gt-link-manager' ), array( 'status' => 500 ) );
		}
	...
	public function bulk_category_action( WP_REST_Request $request ) {
		...
		if ( ! is_array( $link_ids ) || empty( $link_ids ) ) {
			return new WP_Error( 'gtlm_ids_required', __( 'link_ids is required.', 'gt-link-manager' ), array( 'status' => 400 ) );
		}
		...
		if ( $category_id > 0 && ! is_array( $this->db->get_category( $category_id ) ) ) {
			return new WP_Error( 'gtlm_category_not_found', __( 'Category not found.', 'gt-link-manager' ), array( 'status' => 404 ) );
		}

		$link_ids = array_values( array_unique( array_filter( array_map( 'absint', $link_ids ) ) ) );
		if ( empty( $link_ids ) ) {
			return new WP_Error( 'gtlm_ids_invalid', __( 'No valid link IDs provided.', 'gt-link-manager' ), array( 'status' => 400 ) );
		}
	...
	public function create_category( WP_REST_Request $request ) {
		...
		if ( '' === $data['name'] ) {
			return new WP_Error( 'gtlm_category_invalid', __( 'Category name is required.', 'gt-link-manager' ), array( 'status' => 400 ) );
		}

		$id = $this->db->insert_category( $data );
		if ( $id <= 0 ) {
			return new WP_Error( 'gtlm_category_create_failed', __( 'Could not create category.', 'gt-link-manager' ), array( 'status' => 500 ) );
		}
	...
	public function update_category( WP_REST_Request $request ) {
		...
		if ( ! $ok ) {
			return new WP_Error( 'gtlm_category_update_failed', __( 'Could not update category.', 'gt-link-manager' ), array( 'status' => 500 ) );
		}
	...
	public function delete_category( WP_REST_Request $request ) {
		$id = absint( $request['id'] );
		if ( $id <= 0 ) {
			return new WP_Error( 'gtlm_category_invalid_id', __( 'Invalid category ID.', 'gt-link-manager' ), array( 'status' => 400 ) );
		}

		$ok = $this->db->delete_category( $id );
		if ( ! $ok ) {
			return new WP_Error( 'gtlm_category_delete_failed', __( 'Could not delete category.', 'gt-link-manager' ), array( 'status' => 500 ) );
		}
